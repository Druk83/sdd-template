...
Тема версии: **UI-планирование, экранные шаблоны и безопасная миграция reference artifacts**.

Статус публикации: `supported`; immutable tag: `sdd-template-1.9.2`.
Текущий `default_version`: `1.9.2`.
...
- Планирование пользовательского интерфейса после сценариев: пул экранов (screen pool),
  матрица областей отображения (viewport matrix), спецификации экранов и живые
  эталонные прототипы (live reference prototypes)
- Безопасная миграция legacy reference artifacts с inventory, diff и защитой пользовательских файлов
...
раздела сохраняется. Старые полные инструкции Framework без маркеров не
дублируются: установщик сначала показывает расхождение и запрашивает
`replace-legacy` или `keep`. Аналогично проверяется корневой
`.project-structure.json`: структура мета-репозитория заменяется на структуру
потребителя только после подтверждения.
...
Для документов в `docs/requirements/` фреймворк задаёт каталог и расширение
`.md`, но не одно глобальное имя файла. Сначала сохраняй соглашение проекта и
существующие имена, затем используй явное имя из процедуры H6 или выбери
описательное имя. Русские имена документации допустимы; существующие файлы не
переименовывай ради `kebab-case`. `.gitkeep` нужен только пустым каталогам.
...
- **H0** — Конституция (`AGENTS.md`, `.claude/CLAUDE.md`,
  `.github/agents/copilot-instructions.md`)
- **H1** — Взгляды (правила H1.1-H1.13 внутри H0)
