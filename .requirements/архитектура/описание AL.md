...
> **ОБЯЗАТЕЛЬНО:** Перед детальным описанием сценариев агент ДОЛЖЕН сформулировать перечень Application Components и получить подтверждение разработчика.
...
1. Агент анализирует `docs/requirements/архитектура/описание BL.md` (процессы)
2. Анализирует `docs/requirements/структура ПО/стек проекта.md` (технологии)
3. Формулирует перечень Application Components для реализации BP
4. Выводит блок "КАНДИДАТЫ КОМПОНЕНТОВ AL — ТРЕБУЕТСЯ ПОДТВЕРЖДЕНИЕ"
5. Ожидает подтверждения или корректировки от разработчика
