# Technology Layer (TL) — ArchiMate 3.2 (H6)
...
| 4 | Порты | `docs/requirements/структура ПО/ports.md` | Сетевые интерфейсы |
...
## 1.11 Процедура заполнения (подтверждение инфраструктуры TL)

> **ОБЯЗАТЕЛЬНО:** Перед детальным описанием сценариев агент ДОЛЖЕН сформулировать перечень инфраструктурных компонентов и получить подтверждение разработчика.

### Почему это важно

Technology Layer определяет "НА ЧЕМ" работает система:
- Nodes — это серверы/контейнеры, которые нужно развернуть
- System Software — ПО, которое нужно установить и настроить
- Сети и безопасность — конфигурации, которые влияют на доступность

**Ошибка в определении инфраструктуры** требует переконфигурации среды.

### Алгоритм

1. Агент анализирует `docs/requirements/архитектура/описание AL.md` (компоненты)
2. Анализирует `docs/requirements/структура ПО/стек проекта.md` (инфраструктура)
3. Анализирует NFR (availability, RTO/RPO, security)
4. Формулирует перечень инфраструктурных компонентов
5. Выводит блок "КАНДИДАТЫ ИНФРАСТРУКТУРЫ TL — ТРЕБУЕТСЯ ПОДТВЕРЖДЕНИЕ"
6. Ожидает подтверждения или корректировки от разработчика
7. После подтверждения — описывает детальные сценарии TL

### Формат блока подтверждения

> **ВНИМАНИЕ:** Формат ниже — это **структура**, а не готовый пример.
> Агент ОБЯЗАН заполнить placeholder'ы (`<...>`) реальными данными из `docs/requirements/`.

```
КАНДИДАТЫ ИНФРАСТРУКТУРЫ TL — ТРЕБУЕТСЯ ПОДТВЕРЖДЕНИЕ

На основе анализа Application Layer и стека проекта выделена инфраструктура:

1. NODES (вычислительные ресурсы)
   | ID | Node | Тип | Назначение | Размещает AC |
   |----|------|-----|------------|--------------|
   | TN-01 | <name> | container/VM/server | <description> | AC-01, AC-02 |

2. SYSTEM SOFTWARE (системное ПО)
   | ID | Software | Версия | Назначение | Node |
   |----|----------|--------|------------|------|
   | SW-01 | <name> | <version> | <description> | TN-01 |

3. NETWORKS (сети)
   | ID | Network | Тип | Компоненты | Доступ |
   |----|---------|-----|------------|--------|
   | NET-01 | <name> | internal/external | TN-01, TN-02 | <firewall rules> |

4. HIGH AVAILABILITY
   | Компонент | Стратегия | Replicas | Failover |
   |-----------|-----------|----------|----------|
   | <name> | cluster/active-passive | N | auto/manual |

5. BACKUP & RECOVERY
   | Компонент | Стратегия | RTO | RPO |
   |-----------|-----------|-----|-----|
   | <name> | full/incremental | <time> | <time> |

Пожалуйста, подтвердите инфраструктуру или укажите корректировки.
После подтверждения будут описаны детальные сценарии TL.
```

### Правила формирования кандидатов

> Агент извлекает данные **только** из документации проекта, а не из примеров шаблона.

| Источник | Что извлекать | Куда маппить |
|----------|---------------|--------------|
| `docs/requirements/архитектура/описание AL.md` | Application Components (AC-*) | Node |
| `docs/requirements/структура ПО/стек проекта.md` | Runtime, СУБД, брокеры | System Software |
| `docs/requirements/требования/**` | NFR (availability, SLA) | HA стратегия |
| `docs/requirements/требования/**` | NFR (RTO/RPO) | Backup стратегия |
| `docs/requirements/требования/**` | NFR (security, compliance) | Security конфигурация |

### Важно

- Формат блока — это **структура для заполнения**, не готовый пример
- Агент **запрещено** копировать примеры из шаблонов — только извлекать из проекта
- Все имена узлов и ПО берутся из `docs/requirements/`
- Если AL не готов — создать GAP и дождаться завершения этапа [7 AL]
- **Запрещено** добавлять BL/AL термины (бизнес-правила, API логика) на этом этапе
...
**Считать верным:**
- TL сценарии ссылаются на AL и описывают инфраструктуру без бизнес-терминов.
- Указаны HA/backup/security для критичных компонентов.

**Считать неверным:**
- В TL описаны бизнес-процессы или логика приложений.

---

## Критерии готовности

- [ ] Все AL сценарии имеют инфраструктурное покрытие.
- [ ] Описаны HA/backup/security/monitoring.
- [ ] Нет BL/AL терминов.
