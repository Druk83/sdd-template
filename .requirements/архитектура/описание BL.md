...
## 1.8 Процедура заполнения (подтверждение сущностей BL)

> **ОБЯЗАТЕЛЬНО:** Перед детальным описанием процессов агент ДОЛЖЕН сформулировать перечень сущностей Business Layer и получить подтверждение разработчика.

### Почему это важно

Business Layer определяет "ЧТО" делает система с точки зрения бизнеса:
- Business Actors/Roles используются во всех процессах
- Business Objects связываются с Data Objects в структуре данных
- Business Events определяют триггеры и реакции
- Business Processes — основа для Application Layer

**Ошибка в определении сущностей BL** распространяется на AL и TL.

### Алгоритм

1. Агент анализирует `docs/requirements/предметная область.md` (акторы, глоссарий)
2. Анализирует `docs/requirements/домены/` (границы, ответственности)
3. Анализирует `docs/requirements/сценарии/` (процессы, события)
4. Формулирует перечень сущностей BL
5. Выводит блок "КАНДИДАТЫ СУЩНОСТЕЙ BL — ТРЕБУЕТСЯ ПОДТВЕРЖДЕНИЕ"
6. Ожидает подтверждения или корректировки от разработчика
7. После подтверждения — описывает детальные процессы
...
```
КАНДИДАТЫ СУЩНОСТЕЙ BL — ТРЕБУЕТСЯ ПОДТВЕРЖДЕНИЕ

На основе анализа предметной области, доменов и сценариев выделены сущности:

1. BUSINESS ACTORS (кто взаимодействует с системой)
   | ID | Actor | Тип | Описание |
   |----|-------|-----|----------|
   | BA-01 | <name> | external/internal | <description> |

2. BUSINESS ROLES (роли акторов)
   | ID | Role | Actor | Scope |
   |----|------|-------|-------|
   | BR-01 | <name> | BA-01 | <description> |

3. BUSINESS OBJECTS (с чем работает бизнес)
   | ID | Object | Домен | Связь с данными |
   |----|--------|-------|-----------------|
   | BO-01 | <name> | <domain> | -> Object N (БД) |

4. BUSINESS EVENTS (что запускает процессы)
   | ID | Event | Триггер | Инициатор |
   |----|-------|---------|-----------|
   | BE-01 | <name> | <what triggers> | <actor/system> |

5. BUSINESS PROCESSES (предварительный перечень)
   | ID | Process | Домен | Тип | Участники |
   |----|---------|-------|-----|-----------|
   | BP-01 | <name> | <domain> | Use Case/Event-driven | BA-01, BR-01 |

Пожалуйста, подтвердите перечень сущностей или укажите корректировки.
После подтверждения будут описаны детальные сценарии для каждого процесса.
```
...
|----------|---------------|--------------|
| `docs/requirements/предметная область.md` | Раздел "Участники домена" | Business Actor |
| `docs/requirements/предметная область.md` | Раздел "Роли" | Business Role |
| `docs/requirements/сценарии/<domain>/карта процесса.md` | Объекты в шагах UC | Business Object |
| `docs/requirements/сценарии/<domain>/каталог мероприятий.md` | Domain Events | Business Event |
| `docs/requirements/сценарии/<domain>/карта процесса.md` | Use Cases | Business Process |
...
- Все имена акторов, объектов и процессов берутся из `docs/requirements/`
- Если источники неполные — создать GAP и запросить уточнение
- **Запрещено** добавлять AL/TL термины (сервисы, API, инфраструктура) на этом этапе
