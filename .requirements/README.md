...
- `.requirements/системное проектирование/*.md` (H6) - процедуры этапа [7 SD]
...
## 11) Общий протокол GAP для H6

Все H6-процедуры используют единый протокол из `.manifest/gapmanifest.md`:

1. Перед вопросом выполнить `gap.H4.8` и проверить `.source/` и предыдущие
   этапы.
2. Не спрашивать подтверждённый факт; спрашивать только выбор или коллизию.
3. Оформлять вопрос по `gap.H4.9` с источниками, влиянием и действием после
   ответа.
4. После решения выполнить `gap.H4.10` и проверить связанные H8-результаты.
5. Перед закрытием этапа выполнить `gap.H4.11`.

Полный протокол хранится в манифесте, поэтому отдельные H6-шаблоны ссылаются
на него и не дублируют нормативный текст. Примеры в H6 должны быть абстрактными
либо извлекаться из источников текущего consumer-проекта.

### 10.1 Политика имени результата

Фреймворк фиксирует каталог результата и расширение `.md`, но не навязывает
одно глобальное имя для всех процедур. Точное имя выбирается по приоритету:

1. соглашение проекта и уже существующий документ;
2. явное имя в разделе `Выходные данные` конкретной процедуры H6;
3. согласованное описательное имя проекта.

Маска `*.md` означает любой документ результата в указанном каталоге, а не
обязательное имя файла. Существующие документы не переименовываются из-за
имени шаблона. Русские имена допустимы для русскоязычной документации; правило
`kebab-case` применяется к коду, идентификаторам и новым англоязычным именам.
`.gitkeep` нужен только для пустого каталога, который должен быть сохранён в
Git, и не добавляется в каталог с документами.
