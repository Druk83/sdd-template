...
> **Подход:** `.approach/event-storming.md` (концептуальная методология Event Storming).
...
**Правило:** если каталог не проходит минимум — детализируй.
...
> **Критически важно:** НЕ создавай детальные карточки событий без подтверждения разработчика.

### Шаг 1. Извлечение кандидатов событий

Из карты процесса извлеки:
...
После подтверждения:
1. Создай реестр событий в табличном формате
2. Создай детальные карточки для ключевых событий
