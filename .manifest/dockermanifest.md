...
## docker.H4.16 Выбирай offline-зависимости только при обоснованной необходимости
Online dependency flow остается обычным вариантом, если build-среда имеет стабильный доступ к registry и сборка воспроизводится через lock-файл.
**Что делать:**
- Сначала проверь обычный путь: сеть, DNS, proxy, registry mirror и BuildKit cache.
- Если production-сборка не должна зависеть от внешнего registry, оформи offline flow как отдельное решение.
- Создай локальный каталог зависимостей штатным инструментом языка (`cargo vendor`, `go mod vendor`, аналогичный механизм экосистемы).
- Добавь конфигурацию package manager, которая явно направляет build на локальный каталог зависимостей.
- Собирай production-образ в locked/offline-режиме, чтобы build не менял lock-файл и не обращался к registry.
- Включи локальный каталог зависимостей, lock-файл и конфигурацию package manager в релизный артефакт.
- Проверь, что `.dockerignore` и `.gitignore` не исключают файлы, которые package manager считает исходниками vendored-зависимостей.
- Опиши команду обновления локальных зависимостей в документации проекта.
**Запрещено:**
- Переходить на vendored-зависимости без проверки online/cache/mirror вариантов.
- Смешивать online и offline dependency flow без явного правила выбора.
- Коммитить build output (`target`, `dist`, `node_modules`) вместо исходных vendored-зависимостей.
- Удалять lock-файл или отключать locked-режим, чтобы скрыть рассинхронизацию зависимостей.
