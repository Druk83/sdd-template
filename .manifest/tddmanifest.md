...
## 1) Создавай тесты только при наличии задачи или issue
...
## 2) Сначала тест, потом код (Red → Green → Refactor)
...
## 3) Тестируй требования, не детали реализации
...
## 4) Выбирай самый дешёвый уровень теста
...
## 5) Дополняй существующие тесты, не создавай новые без причины
...
## 6) Обеспечь детерминизм и изоляцию тестов
Тесты должны быть:
...
## 7) Мокай только границы, не доменную логику
...
## 8) Не лечи падения ослаблением тестов
...
## 9) Проверяй и адаптируй релевантные старые тесты
...
## 10) Стандартизируй запуск тестов
...
## 12) Проверь Definition of Done теста
...
## 13) Добавляй мини-карточку к каждому новому тесту
