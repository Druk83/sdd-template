...
### 4.6 Работа с placeholder'ами

При создании документа агент часто не знает конкретных значений (имена доменов на этапе [1]).

**Правило:**
1. Ставить маркер `<!-- @update-after:[N] Описание -->` перед placeholder'ом
2. После завершения этапа [N] — найти все маркеры и заменить на реальные значения
3. Если placeholder не может быть заполнен — создать GAP
...
### 4.7 Точки подтверждения кандидатов
