...
### 4.5 Критерии детализации артефактов

**ОБЯЗАТЕЛЬНО:** Перед созданием артефакта агент изучает примеры GOOD/BAD в шаблоне.

Каждый шаблон в `.requirements/` содержит:
1. **Пример GOOD** — достаточная детализация, на которую нужно ориентироваться
2. **Пример BAD** — недостаточная детализация, которую нельзя допускать
...
**Правило:** Если артефакт похож на пример BAD — агент ОБЯЗАН детализировать до уровня GOOD.
