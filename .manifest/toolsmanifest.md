...
| `version` | Текущая версия инструмента в формате SemVer |
...
- `availability` - доступность инструмента: `bundled` или `external-optional`
- `install_manifest` - путь к манифесту внешнего пакета, если инструмент опциональный

Если `availability=external-optional`, отсутствие указанного `path` не считается
ошибкой репозитория. Перед запуском агент проверяет `install_manifest` и сообщает
о необходимости установки готового пакета по его инструкциям.
...
- [ ] Версия указана в README инструмента в установленном формате
- [ ] Версия в `registry.json` совпадает с версией в README
- [ ] Есть `--help`
...
## tools.H3.7 Версионируй каждый инструмент
Для формата SemVer и общих значений `MAJOR`, `MINOR`, `PATCH` используй
`.manifest/versionmanifest.md` (`version.H3.3`, `version.H4.3`).
Эти правила относятся к версии набора шаблонов; версия инструмента является отдельной.

README каждого инструмента является каноническим источником его версии.
Версия указывается отдельной строкой в формате:

```markdown
> Версия инструмента: `X.Y.Z`.
```

Поле `version` в `.tools/registry.json` дублирует версию из README
и используется как быстрый справочник версий всех доступных инструментов.
Исходный код инструмента не должен содержать hardcoded-версию или объявление
вида `version = ...`.

## tools.H4.11 Правила increment версии
Для инструментов применяй общий SemVer с учетом изменений их CLI-контракта:

| Уровень | Когда увеличивать |
|---------|-------------------|
| `MAJOR` | Несовместимое изменение CLI, входных данных, выходных форматов или поведения |
| `MINOR` | Новая обратно совместимая возможность, опция или поддерживаемый формат |
| `PATCH` | Исправление ошибки, уточнение документации или внутреннее изменение без изменения интерфейса |

При изменении версии сначала обновляй строку в README инструмента, затем поле
`version` в `.tools/registry.json`.
Не используй `latest`, плавающие значения и версии, не подтвержденные исходными файлами.
