...
**Уровни:**
- H3 - политики (общие принципы, императивы верхнего уровня)
- H4 - стандарты (конкретные императивы с форматом/командой/шаблоном)
...
|---------|---------------|---------------|
| Docker | "Не храни секреты в образах" | "Передавай секреты через `docker secret` или `--env-file`" |
| PDD | "Документируй незавершенный код" | "Используй формат `@todo #TASK-ID:ESTIMATE Description`" |
| TDD | "Сначала тест, потом код" | "Называй тесты по шаблону `test_<method>_<scenario>_<expected>`" |
| Dependencies | "Не устанавливай вслепую" | "Проверяй зависимости командой `npm audit --production`" |
...
| Область | Хорошо | Плохо |
|---------|--------|-------|
| Зависимости | "Проверяй командой `npm audit`" | "Команда аудита" |
| Docker | "Фиксируй версию образа" | "Формат версии" |
| TDD | "Называй тесты по шаблону" | "Naming convention" |
| Git | "Используй `.gitignore` для секретов" | "Файлы исключений" |
...
Манифесты **автономны относительно фактов проекта**:
- НЕ зависят от `.requirements/` или `docs/` как источников данных
- НЕ требуют знания конкретного проекта для понимания правил
- Читаются при работе с конкретной областью

**Допустимые ссылки:**
- На другие манифесты как нормативные правила (например, `toolsmanifest.md` -> `readmemanifest.md`)
- На справочники команд/инструментов
- На внешние стандарты (ГОСТ, RFC, etc.)

**Недопустимые ссылки:**
- На конкретные артефакты проекта (`docs/requirements/...`)
- На данные из `.source/`
