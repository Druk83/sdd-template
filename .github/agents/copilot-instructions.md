# Rules and Constraints for Copilot (H0)

## ВАЖНЕЙШИЕ ВЗГЛЯДЫ (H1)

### H1.1. НИКОГДА не использовать эмодзи
**Запрещено использовать эмодзи в:**
- Коде (comments, переменные, строки)
- Файлах (Markdown, README, задачи)
- Сообщениях пользователю
- Git commit messages (если предлагаю текст)

### H1.2. Секреты и credentials
**Запрещено:**
- Хардкодить секреты, API ключи, пароли в коде
- Коммитить `.env` файлы с реальными значениями
- Логировать sensitive данные
**Используй:**
- Переменные окружения
- `.env.example` для плейсхолдеров
- `.gitignore` для исключения `.env`

### H1.3. ТОЛЬКО пользователь делает git commit/push
**Запрещено тебе:**
- Создавать коммиты (`git commit`)
- Делать push (`git push`)
- Создавать ветки (`git checkout -b`)
- Делать merge/rebase
- Изменять git историю
**Можешь:**
- Создавать и редактировать файлы
- Запускать `git status`, `git diff` для проверки
- Предлагать текст commit message (без эмодзи)
**Порядок работы:**
1. Создавай/редактируй файлы
2. Проверяй изменения: `git status`, `git diff`
3. Предлагай описание изменений
4. **Пользователь сам делает `git add`, `git commit`, `git push`**

### H1.4. Верификация релевантности контексту
**Запрещено:**
- Генерировать термины/примеры из обучающих данных
- Использовать примеры из других доменов (если не релевантны проекту)
- Заполнять placeholder'ы "типичными" примерами
**Используй:**
- Термины из входных источников (`.source/`, документация)
- Примеры из контекста текущего проекта
- При сомнении спрашивай разработчика
**Проверка:** можно ли проследить термин до источника в проекте?

## H1.5. Стиль общения
### Язык
- Отвечай на русском языке
- Используй код и технические термины на английском
- Называй файлы, переменные, функции на английском
### Тон
- Давай краткие и четкие ответы
- Используй технический язык без восторгов
- Сохраняй объективность вместо похвалы
- Предоставляй факты и решения, а не эмоции
- Не используй "Вы абсолютно правы" или подобные фразы
### Формат ответа
- Давай прямые ответы на вопросы
- Структурируй информацию (списки, таблицы)
- Оформляй примеры кода в блоках ```
- Не используй эмодзи

## H1.6. Файловые операции
### Создание файлов
- Создавай файлы только когда необходимо
- Предпочитай редактирование существующих файлов
- Не создавай .md файлы без явного запроса пользователя
- Используй абсолютные пути
### Очистка временных файлов
- ВСЕГДА удаляй временные файлы после завершения задачи
- Удаляй промежуточные файлы, созданные в процессе выполнения команд
- Не оставляй мусор: temp-*, *.tmp, промежуточные скрипты
- Проверяй рабочую директорию перед завершением задачи
### Комментарии к коду
- Пиши комментарии на русском языке (код на английском)
- Не используй эмодзи
- Будь кратким и по существу
- Комментируй только неочевидную логику
### Присвоение имен и документация
- Называй переменные, функции, классы на английском
- Пиши JSDoc/annotations для публичного API на английском
- Пиши JSDoc/annotations для внутреннего кода на русском
- Используй type annotations на английском (это часть кода)
- Для исходного кода и новых англоязычных файлов используй kebab-case: `user-service.ts`, `auth-controller.ts`.
- Имена проектной документации выбирай по соглашению проекта. Русские имена документов допустимы; существующие документы не переименовывай только ради kebab-case.
- Называй ветки в формате type/description: `feature/add-auth`, `fix/login-bug`, `refactor/user-module`

## H1.7. Обработка ошибок
**При ошибках выполнения команд:**
- Сообщи пользователю о проблеме
- Предложи варианты решения
- Не повторяй упавшую команду без изменений
**При отсутствии файла/директории:**
- Проверь правильность пути
- Спроси пользователя о корректном расположении
- Не создавай файлы/директории без подтверждения
**При неоднозначности задачи:**
- Уточни требования у пользователя
- Не делай предположений о намерениях

## H1.8. Базовая проектная документация
Используй документацию проекта из:
- `.requirements/` в мета-репозитории или `FRAMEWORK_ROOT/.requirements/` в проекте-потребителе — шаблоны и методики для написания документации (read-only)
- `docs/requirements/` - готовые документы технической документации, которые создает агент/разработчик
- `.tasks/` - текущие задачи и планирование
- `.issues/` - текущие проблемы и баги
- `.manifest/` в мета-репозитории или `FRAMEWORK_ROOT/.manifest/` в проекте-потребителе - манифесты инструментов и процессов
- `README.md` - обзор проекта
**Соответствие шаблонов и результатов в docs/:**
| Шаблон (read-only) | Результат |
|--------------------|-----------|
| `.requirements/предметная область.md` или `FRAMEWORK_ROOT/.requirements/предметная область.md` | `docs/requirements/предметная область.md` |
| `.requirements/требования/*.md` или `FRAMEWORK_ROOT/.requirements/требования/*.md` | `docs/requirements/требования/<имя-документа>.md` |
| `.requirements/домены/*.md` или `FRAMEWORK_ROOT/.requirements/домены/*.md` | `docs/requirements/домены/<имя-документа>.md` |
| `.requirements/сценарии/*.md` или `FRAMEWORK_ROOT/.requirements/сценарии/*.md` | `docs/requirements/сценарии/<domain>/<имя-документа>.md` |

Путь и расширение результата являются обязательным контрактом. Точное имя
файла не задаётся глобальной маской: сначала используй соглашение проекта,
затем явное имя из выбранной процедуры H6, если оно указано, иначе выбери
описательное имя и сообщи его в отчёте. Не переименовывай существующий
результат только из-за имени шаблона. `.gitkeep` добавляй только в пустой
каталог, который необходимо сохранить в Git.
**При работе с задачами:**
- Читай `.requirements/` в мета-репозитории или `FRAMEWORK_ROOT/.requirements/` в проекте-потребителе для понимания шаблонов
- Читай `docs/requirements/` для фактов проекта
- Проверяй `.tasks/` для актуальных задач
- Учитывай `.issues/` для известных проблем
- Следуй архитектурным решениям из документации

## H1.9. Структура проекта
Прочитай `.project-structure.json` в корне репо перед началом работы.
В проекте-потребителе определи `FRAMEWORK_ROOT` по единственной папке
`.agents/sdd-template-X.Y.Z/` и используй его для ресурсов Framework.
В нем:
- Иерархия основных директорий
- Назначение каждой папки
- Ключевые файлы по функциональности
Используй эту информацию для навигации по коду вместо вызова tools.
**Если файл отсутствует:**
- Спроси пользователя о структуре проекта
- Не пытайся угадывать структуру самостоятельно

## H1.10. Работа с манифестами
**Перед работой в любой области:**
Проверь наличие манифеста в `.manifest/<область>manifest.md` мета-репозитория
или в `FRAMEWORK_ROOT/.manifest/<область>manifest.md` проекта-потребителя и изучи его.
**Приоритет при конфликте:**
1. Локальный манифест в директории компонента (`./<component>/manifest.md`)
2. Глобальный манифест (`.manifest/<название>manifest.md`)
3. Инструкции из этого файла (copilot-instructions.md)

## H1.11 Применяй правильно иерархию в документах
Прочитай `.manifest\hierarchymanifest.md` в мета-репозитории или
`FRAMEWORK_ROOT/.manifest/hierarchymanifest.md` в проекте-потребителе. В нём
определены политики иерархии документации в проекте.

## H1.12. Логирование сессии чата
Разработчик использует инструмент логирования по требованию.
**Когда применять:** только при явном запросе разработчика ("сохрани логи", "запиши логи" и т.п.).
**Правила:** читай `.manifest/chatlogmanifest.md` в мета-репозитории или
`FRAMEWORK_ROOT/.manifest/chatlogmanifest.md` в проекте-потребителе перед выполнением.
**Артефакты:** `.chatlog/session.md` в мета-репозитории либо
`FRAMEWORK_ROOT/.chatlog/session.md` в проекте-потребителе (по умолчанию), или
файл с именем, указанным разработчиком. Корневой `.chatlog/` проекта-потребителя
для журналов Framework не создавай.

## H1.13. Формат употребления англицизмов
**Область правила:**
- Правила языка, технических терминов, имён и релевантности уже определены в H1.4, H1.5 и H1.6.
- Этот раздел устанавливает только единый формат употребления англицизмов в русскоязычной документации.
**Первое упоминание:**
- При первом упоминании указывай русский термин и английский оригинал: `матрица влияния (impact matrix)`.
- Если русский эквивалент отсутствует или искажает смысл, укажи английский термин и краткое русское пояснение.
**Последующее использование:**
- В пределах одного документа используй выбранную форму последовательно.
- Не смешивай без основания варианты `матрица влияния`, `impact matrix` и `impact-матрица`.
- Идентификаторы, статусы, имена и пути оформляй по правилам H1.5 и H1.6.
